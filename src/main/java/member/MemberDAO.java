...
//  회원 조회
	public List<MemberVO> listMembers(){
		...
		try {
	        ...
	        String query = "select * from memtbl order by joinDate desc";
...
//  회원 등록
	  public void addMember(MemberVO memberVO) {
	     try {
	        ...
	        int phone1= memberVO.getPhone1();
	        int phone2= memberVO.getPhone2();
	        int phone3= memberVO.getPhone3();
	        String name = memberVO.getName();
	        String query = "insert into memtbl (email, pwd, postcode, address, detailAddress, phone1, phone2, phone3, name) values(?,?,?,?,?,?,?,?,?)";
	        pstmt = conn.prepareStatement(query);
	...
	//  수정할 회원정보 찾기
	  public MemberVO findMember(String _email) {
	     ...
	     try {
        	...
        	String name = rs.getString("name");
        	Date joinDate = rs.getDate("joinDate");
	        memFindInfo = new MemberVO(email, pwd, postcode, address, detailaddress, phone1, phone2, phone3, name, joinDate);
	        pstmt.close();
	        conn.close();
	        rs.close();
	     } catch (Exception e) {
	  ...
	  public void modMember(MemberVO memberVO) {
		  String email = memberVO.getEmail();
	        ...
	        String name = memberVO.getName();
		   	try {
		   			conn=dataFactory.getConnection();
		   			String query="update memtbl set pwd=?,postcode=?,address=?,detailAddress=?,phone1=?,phone2=?, phone3=? where email=?";
		   			pstmt=conn.prepareStatement(query);
